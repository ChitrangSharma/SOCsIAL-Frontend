...
export default function SubscribedUserPosts    () {
  ...
  useEffect(()=>{
    ...
    .then(result=>{
      console.log( "This is the result find ",result)
  ...
  return (
    ...
    {data.map(item =>{
        ...
        action={
          <IconButton aria-label="settings">
            {item.postedBy._id == state._id 
               && <HighlightOffIcon onClick={()=>deletePost(item._id)} style={{float:"right"}} />
               }
          </IconButton>
                 }
                //  isssue iam not getting the name who posted 
      ...
      <CardActions disableSpacing>   
        {item.likes.length === 0?
          <FavoriteIcon style={{marginLeft:"10px",color:" #4D484C"}} />
        :
       ...
       {/*  #4D484C gray color */}
               {item.smile.length === 0?
                <SentimentDissatisfiedIcon style={{marginLeft:"10px",color:"#4D484C"}} />
               :
               <EmojiEmotionsIcon style={{marginLeft:"10px",color:"yellow"}}/>
               } 
      ...
      <Collapse in={expanded} timeout="auto" unmountOnExit>
...

        {
                     ...
                     {
                       return(
                        <Typography key={record._id}><span style={{fontWeight:"500"}}><strong>{record.postedBy.name}</strong></span> {record.text}
